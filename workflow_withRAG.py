...
from openevolve_graph.get_logger import DetailedLogger 
...
parser.add_argument(
    ...
    type = str,
    help = "检查点路径",
    default = "./circle_packing/checkpoint_28"
)
...
def main(dict_state: dict):
    ...
    while iterations < config.max_iterations:
        ...
        for island_state in main_graph_state.islands.values():
            main_graph_state.islands[island_state.id].now_meeting = 0
            main_graph_state.islands[island_state.id].next_meeting = meeting_interval
        if config.use_rag:
            try:
                main_graph_state=asyncio.run(rag_node.execute(main_graph_state))
                
            except Exception as e:
                logger.error(f"RAG节点执行失败: {e}")
