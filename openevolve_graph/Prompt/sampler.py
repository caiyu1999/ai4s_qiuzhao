...
class PromptSampler_langchain:
    ...
    def build_prompt(
        ...
        **kwargs: Any,                       # 其他参数
        ...
        logger.debug(f"evolution_history:{evolution_history}")
        # 如果启用并且可用，格式化工件部分
        artifacts_section = ""
    ...
    def _format_evolution_history(
        ...
        logger.debug(f"previous_programs:{previous_programs}")
        logger.debug(f"top_programs:{top_programs}")
        logger.debug(f"inspirations:{inspirations}")
